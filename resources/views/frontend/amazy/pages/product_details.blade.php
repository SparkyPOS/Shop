                                ...
                                <div class="viendor_text d-flex align-items-center">
                                    <p class="stock_text"> <span class="text-uppercase">{{__('defaultTheme.availability')}}:</span> <span class="stock_value" id="availability">
                                        ...
                                        @endif
                                                                        ...
                                                                        <div class="viendor_text d-flex align-items-center">
                                                <p class="f_w_500 m-0"> <span class="text-uppercase">{{ __('common.listed_date') }}: {{dateConvert(@$product->product->created_at)}}</p>
                                </div>
                                ...
                                <div class="col-12">
                                    <div class="product_details_dec mb_76">
                                        <div class="product_details_dec_header">
                                            <h4 class="font_18 f_w_700 m-0">{{__('common.description')}}</h4>
                                        </div>
...
//                                        ->where('created_by', $product->user_id)
                    ...
                    <div class="amazcart_delivery_wiz mb_20">
                        <div class="amazcart_delivery_wiz_body">
                            <div class="amazcart_delivery_wiz_sep d-flex gap_15 mb_10">
                                <div class="icon d-flex align-items-center justify-content-center ">
                                    <img src="{{url('/')}}/public/frontend/amazy/img/product_details/return-box.svg" alt="{{__('amazy.Return Policy')}}" title="{{__('amazy.Return Policy')}}">
                                </div>
                                <div class="amazcart_delivery_wiz_content">
                                    <h4 class="font_16 f_w_700 mb_6">{{__('amazy.Return Policy')}}</h4>
                                    <p class="delivery_text font_14 f_w_400">
                                        {{__('amazy.Click The Following Link To View Details On Return Policy.')}} <a class="text-nowrap" href="{{url('/return-exchange')}}">{{__('common.see_more')}}.</a>
                                    </p>
                                </div>
                            </div>
                        </div>
                                ...
                                <h4 class="font_14 f_w_700 mb-0">
                                    <a id="shopLink" href="
                                        @if ($product->seller->slug)
                                            {{route('frontend.seller',['seller_id'=>$product->seller->slug,'vendor_id'=>@$product->seller->SellerAccount->vendor_id])}}
                                        @else
                                            {{route('frontend.seller',base64_encode($product->seller->id))}}
                                        @endif
                                    ">
                                        @if($product->seller->role->type == 'seller')
                                            @if (@$product->seller->SellerAccount->vendor_id)
                                                {{__('amazy.Vendor ID')}}: {{ @$product->seller->SellerAccount->vendor_id }}
                                            @else
                                                {{$product->seller->first_name .' '.$product->seller->last_name}}
                                            @endif
                                        @else
