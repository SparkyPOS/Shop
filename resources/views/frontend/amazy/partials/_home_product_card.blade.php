@php
    ...
    if (isModuleActive('AuctionProducts') && isset($product->auction) && $product->auction) {
        ...
        if ($isAuctionActive) {
            ...
            $startDate = date('Y/m/d', strtotime($auction->auction_start_date));
            $endDate = date('Y/m/d', strtotime($auction->auction_end_date));
...
<div class="{{ $cardClass }} {{ $isAuctionActive ? 'home-auction-card home-auction-card--' . $auctionStyle : '' }}">
    ...
    <div class="product_thumb_upper">
        ...
        @if(!$isAuctionActive && isGuestAddtoCart())
    ...
    <div class="{{ $metaClass }}">
        ...
        <a href="{{ singleProductURL(@$product->seller->slug, $product->slug) }}">
            <h4>
                ...
                @else
                    {{ textLimit(@$product->product->product_name, 50) }}
                @endif
            </h4>
        </a>
        ...
        @if($isAuctionActive)
            <div class="product_price d-flex align-items-center justify-content-between flex-wrap">
                ...
                <p>
                    <strong>{{ single_price($auction->starting_bidding_price) }}</strong>
                </p>
            </div>
