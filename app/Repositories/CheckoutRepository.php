            ...
            foreach($cartData as $seller_id => $packages){
                ...
                foreach($packages as $cart_key => $cart){
                    ...
                    if($cart->product_type == 'product'){
                        if (app('general_setting')->price_with_vat && app('gst_config')['enable_gst'] == "only_tax"){
                        ...
                        if(file_exists(base_path().'/Modules/GST/') && $cart->product->product->product->is_physical == 1){
                            ...
                            elseif(app('gst_config')['enable_gst'] == "flat_tax"){
                                if($cart->product->product->product->gstGroup){
                                    ...
                                    $percentage = !empty($flatTax->tax_percentage) ? $flatTax->tax_percentage:0;
                                    $gstAmount =  $cart->total_price * $percentage / 100;
                                    $tax += $gstAmount;
                                }
                            }elseif(app('gst_config')['enable_gst'] == "only_tax"){
                                if($cart->product->product->product->gsttax ){
                                    $percentage = !empty($cart->product) && !empty($cart->product->product) && !empty($cart->product->product->product) && !empty($cart->product->product->product->gsttax) && !empty($cart->product->product->product->gsttax->tax_percentage) ? $cart->product->product->product->gsttax->tax_percentage:0;
                                    $gstAmount = $cart->total_price * $percentage / 100;
                                    $tax += $gstAmount;
                                }
                            }
                            ...
                            if($cart->product->product->product->gstGroup){
                                ...
                                foreach ($sameStateTaxes as $key => $sameStateTax){
                                    $percentage = !empty($sameStateTax) && !empty($sameStateTax->tax_percentage) ? $sameStateTax->tax_percentage:0;
                                    $gstAmount = ($cart->total_price * $percentage) / 100;
                                    $tax += $gstAmount;
                                }
                            }
