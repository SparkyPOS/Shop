...
class WishlistRepository
{
    ...
    public function removeForAPI($id, $type, $user_id){
        $product =  Wishlist::where('user_id', $user_id)->where('id', $id)->where('type', $type)->first();
        if($product){
            $product->delete();
            return true;
        }else{
    ...
    public function myWishlistAPI($user_id){
        $wishList = Wishlist::with('user', 'seller','giftcard', 'product', 'product.product')->where('user_id',$user_id)->get();
        return WishListResource::collection($wishList)->groupBy('seller_id');
    }
