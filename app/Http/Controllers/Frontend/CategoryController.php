...
//        $item = $request->item?$request->item:'search';
        $item = 'search';
        $slug='as';
//        dd($slug);
        if ($item == 'search') {
            $searchTerm = SearchTerm::where('keyword','')->first();
            if($searchTerm){
            ...
            $slugs = explode(' ',$slug);
            $mainProducts = Product::when(count($slugs) > 0,function($q) use ($slugs){
//                $q->whereHas('tags', function($q) use($slugs){
