        ...
        <form method="POST" action="{{route('seller.profile.return-address.update',$seller->id)}}" id="return_address_form" accept-charset="UTF-8" class="form-horizontal" enctype="multipart/form-data">
            ...
            <div class="white-box">
                <div class="add-visitor">
                                ...
                                <ul id="theme_nav" class="permission_list sms_list ">
                                    <li>
                                        <label data-id="bg_option" class="primary_checkbox d-flex mr-12">
                                            <input name="same_as_warehouse" id="status_active" value="1" {{ optional($seller->SellerReturnAddress)->same_as_warehouse == 1 ? 'checked' : '' }} class="active same_address_check" data-seller_id="{{$seller->id}}" type="radio">
                                    ...
                                    <li>
                                        <label data-id="color_option" class="primary_checkbox d-flex mr-12">
                                            <input name="same_as_warehouse" value="0" {{ optional($seller->SellerReturnAddress)->same_as_warehouse == 0 ? 'checked' : '' }}   id="status_inactive" class="de_active same_address_check" type="radio" data-seller_id="{{$seller->id}}">
                                            <span class="checkmark"></span>
                                        </label>
                                        <p>{{ __('common.no') }}</p>
                                    </li>
                    ...
                    <div id="return_div" class="row {{ optional($seller->SellerReturnAddress)->same_as_warehouse == 1 ? 'd-none' : '' }}">
                        <div class="col-xl-6">
                            <div class="primary_input mb-25">
                                ...
                                <input name="name" class="primary_input_field" placeholder="-" type="text"
                                       value="{{ old('name') ? old('name') : optional($seller->SellerReturnAddress)->return_name }}">
                                       @error('name')
                        ...
                        <div class="col-xl-6">
                            <div class="primary_input mb-25">
                                ...
                                <input name="address" class="primary_input_field" placeholder="-" type="text"
                                       value="{{ old('address') ? old('address') : optional($seller->SellerReturnAddress)->return_address }}">
                                       @error('address')
                                       <span class="text-danger">{{$message}}</span>
                                       @enderror        
                            </div>
                        ...
                        <div class="col-xl-6">
                            <div class="primary_input mb-25">
                                ...
                                <input name="phone" class="primary_input_field" placeholder="-" type="text"
                                       value="{{ old('phone') ? old('phone') : optional($seller->SellerReturnAddress)->return_phone }}">
                                       @error('phone')
                                       <span class="text-danger">{{$message}}</span>
                                       @enderror        
                            </div>
                        ...
                        <div class="col-xl-6">
                            ...
                            <select name="country" id="return_country" class="primary_select mb-25">
                                ...
                                <option {{ optional($seller->SellerReturnAddress)->return_country == $country->id ? 'selected' : '' }} value="{{$country->id}}">{{$country->name}}</option>
                                @endforeach
                            </select>
                            @error('country')
                                <span class="text-danger">{{$message}}</span>
                        ...
                        <div class="col-xl-6">
                            ...
                            <select name="state" id="return_state" class="primary_select mb-25">
                                <option value="" disabled selected>{{__('common.select_one')}}</option>
                                @php($states = optional(optional($seller->SellerReturnAddress)->country)->states ?? [])
                                @foreach($states as $key => $state)
                                    <option {{ optional(optional($seller->SellerReturnAddress)->state)->id == $state->id ? 'selected' : '' }} value="{{$state->id}}">{{$state->name}}</option>
                                @endforeach
                            </select>
                            @error('state')
                                <span class="text-danger">{{$message}}</span>
                        ...
                        <div class="col-xl-6">
                            ...
                            <select name="city" id="return_city" class="primary_select mb-25">
                                <option value="" disabled selected>{{__('common.select_one')}}</option>
                                @php($cities = optional(optional($seller->SellerReturnAddress)->state)->cities ?? [])
                                @foreach($cities as $key => $city)
                                    <option {{ optional(optional($seller->SellerReturnAddress)->city)->id == $city->id ? 'selected' : '' }} value="{{$city->id}}">{{$city->name}}</option>
                                @endforeach
                            </select>
                            @error('city')
                                <span class="text-danger">{{$message}}</span>
                        ...
                        <div class="col-xl-6">
                            ...
                            <input name="postcode" class="primary_input_field" placeholder="-" type="text"
                                   value="{{ old('postcode') ? old('postcode') : optional($seller->SellerReturnAddress)->return_postcode }}">
